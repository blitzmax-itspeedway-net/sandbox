	...
	Method New( form:Object, fx:Int, fy:Int )
		...
		For Local fld:TField = EachIn t.EnumFields()
			...
			Select fldType
				...
				Default
'DebugStop
					Local fldtypeid:TTypeId = fld.TypeID()
					'Else If fldtype.isStruct()
					'	DebugStop
					'End If
					If fldtypeid.isClass()
						fields.addlast( MakeLabel( "(class)" ) )
					'If Object( fld )
					'	fields.addlast( MakeLabel( "(object)" ) )
					'ElseIf fldtypeid.IsArray()
					'	fields.addlast( MakeLabel( "(array)"  ) )
					'ElseIf fld.IsArray()
					'	fields.addlast( MakeLabel( "(array)"  ) )
					ElseIf fldtypeid.IsEnum()
						fields.addlast( MakeLabel( "(enum)"  ) )
					'ElseIf fldtypeid.IsInterface()
					'	fields.addlast( MakeLabel( "(interface)"  ) )
					ElseIf fldtypeid.IsStruct()
						fields.addlast( MakeLabel( "(struct)" ) )
					Else
