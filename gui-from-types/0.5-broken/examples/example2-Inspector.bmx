...
'	EXAMPLE SHOWING OBJECT VIEW AND INSPECTION
...
Global RED:SColor8 = New SColor8( $ff0000 )
...
Type TBlob
	...
	Method New()
		autonumber :+ 1
		id = autonumber
		colour = New SColor8( Rand( 0,255), Rand( 0,255), Rand( 0,255) )
		radius = Rand( 5, 15 )
	...
	Method update( delta:Float )
		If boundsCheck( Int(pos.x-radius), Int(pos.y-radius), radius*2, radius*2 )
	...
	Method render()
		SetColor( TForm.iif( mouseover=Self, RED, WHITE ) )
...
' CREATE A FORM TO HOLD THE INSPECTOR
...
Repeat
	...
	' CREATE OR VIEW THE INSPECTOR
	If TBlob.mouseOver And MouseHit( 1 )
		inspector = New TForm( TBlob.mouseOver, MouseX(), MouseY() )
	End If
	' SHOW INSPECTOR / RETURNS FALSE WHEN CLOSED
	If inspector And Not inspector.show(); inspector = Null
