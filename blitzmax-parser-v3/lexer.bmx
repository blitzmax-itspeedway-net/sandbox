...
Rem PLAN
When the source changes, the parser still needs a pointer to the original text.
You don't want the lexer updating the parser for every character typed.

When the parser says getnext() if only gets back an integer for the token. 
For literals etc, use gettoken() which doesn't have row/column location, but 
instead has an index into an array. 
That array contains the struct containing the row/column and the lexer maintains this.

We could also use the ESC type lookup for invalidated tokens that are moved to an 
unused array when deleted.

In this way the parser will know what has changed without having to update it's indexes. 

On text change, the lexer tokenises / invalidates the updated section and updates the indexes. 
Then notifies the parser about invalid tokens. The parser re-parses the block.
...
Type TLexer 'Extends TLexerV1
	...
	Field source:String
	Field tokens:TArrayList<TTOken>
	...
	Method New( source:String )
		Self.source = source
		linenum = 1 ; linepos = 0
		tokens = New TArrayList<TToken>()
		cursor = 0
	End Method
	
	' Getnext ONLY returns the token; See also getToken()
	Method getNext:Int()
	End Method
	
	' GetTOken returns an entire token; See also getNext()
	Method getToken:TToken()
	End Method
...
' A Simple Symbol
Type TToken
	Field index:Long			' Used by Lexer to reference the source token
	Field invalid:Int = False	' Used to invalidate tokens during editing
	Field succ:TToken			' Next token
	'
	Field linenum:Int, linepos:Int
End Type
