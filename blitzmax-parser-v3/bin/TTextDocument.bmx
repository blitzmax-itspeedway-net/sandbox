...
Type TFullTextDocument Extends TTextDocument
	...
	Method offsetAt:UInt( position:TPosition )
		...
		If position.linenum >= lineOffsets.length Return content.length
		If position.linenum < 0 Return 0
		Local lineOffset:UInt = lineOffsets[ position.linenum ]
		Local nextLineOffset:UInt
		If position.linenum + 1 < lineOffsets.length
			nextLineOffset = lineOffsets[position.linenum + 1]
		Else
			nextLineOffset = content.length
		End If
		Return Max( Min( lineOffset + position.linepos, nextLineOffset), lineOffset)
	End Method
