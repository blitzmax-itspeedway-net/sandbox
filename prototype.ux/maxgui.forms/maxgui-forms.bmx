...
Include "bin/maxgui.bmx"		
Include "bin/TElement.bmx"
...
'Public
	...
	Method Load:TForm( file:String, msghandler( text:String, options:Int )=Null )
'	DebugStop
		Self.msgHandler = msgHandler
		If FileType( file )<>1 Then
			Writeln( "Unable to load file '"+file+"'.", TEXTFORMAT_RED )
			Return Null
		End If
		Select Lower( ExtractExt( file ))
		Case "bfd" ; parser = New TFormParserBFD.Create( Self )	'# BlitzMAX Form Definition
'		Case "bmf" ; parser = New TFormParserBMF.Create( Self )	'# BlitzMAX Form (Supports Sizers) -EXPERIMENTAL
'		Case "frm" ; parser = New TFormParserFRM.Create( Self )	'# VB6 Form Definition  ## EXPERMIMENTAL ##
'		Case "lfm" ; parser = New TFormParserLFM.Create( Self )	'# LAZARUS FORM
		Default
			Writeln( "Invalid file type '"+file+"'.", TEXTFORMAT_RED )
		End Select
		If Not parser Then Return Null
		'#
		If Not parser.Load( file, msghandler ) Then 
			Writeln( "Failed to parse file '"+file+"'.", TEXTFORMAT_RED )
			Return Null
		End If
		handler = Self
	Return Self
