'# ==================================================================
'# Project Title: Max-GUIde
'# Author: 
'# Based on GUIde V1.4 by wdw@home.nl
'# ==================================================================

Rem CONVERSION NOTES
Anything I comment out is marked '!!
End Rem
...
Include "includes/grib.bmx"					' grid functions
Include "includes/math2d.bmx"				' handy math

Include "guideexportinclude.bmx"		' export Include.... hier moet ik vanaf. wat gebruik ik in Main editor?? groupbox

Include "GUIDEconstants.bmx"					' all constants
Include "UserInterface.bmx"					' builds the GUIDE interface
Include "GadgetsMain.bmx"					' form gadget creation And handling
Include "GadgetsDraw.bmx"					' form gadgets draw functions
Include "MenusMain.bmx"						' form menus creation And handling
Include "LinkedLists.bmx"					' linked lists creation And handling

Include "BlitzPlusExport.bmx"				' export form To bplus code
Include "MaxExport.bmx"							' export form To bmax code
'include "WinBlitz3DExport.bmx"				
...
Function SetUpEditor()
	...
	SelectTreeViewNode editor.rootGT.node
...
Function CloseWindow( source )
	...
	Select source
		Case MAINWINDOW
			...
			' quit
			SettingsSave()
			FreeTreeViewNode editor.rootgt.node
			'!!Delete Each gtype
			For Local U = EachIn gtype ; u=Null; Next
			'!!Delete Each list
			For Local U = EachIn list ; u=Null; Next
			'!!Delete editor
			For Local U = EachIn editor ; u=Null; Next
...
Function DoGadgetAction( gadget )
	...
	Select gadget
		...
		Case AREATXT
			RequestColor( editor.selection.gt.r,editor.selection.gt.g, editor.selection.gt.b )
			editor.selection.gt.textr = RequestedRed()
			editor.selection.gt.textg = RequestedGreen()
			editor.selection.gt.textb = RequestedBlue()
			ShowGadgetDetails( editor.selection.gt )
			...
			Select editor.mode
				...
				Case MODE_MOVEBOTTOM
					...
					If gt.version = FOLDBOX1
						...
						While child.nxt <> Null
							nextypos = child.gt.ypos + child.gt.height
							ymoved = nextypos - child.nxt.gt.ypos
	...
	Select gt.version
		...
		Case WINDOW
			Select item
				Case 0
					ShowGadget PROPTEXT
					SetGadgetText PROPTEXT, gt.name$
				...
				Case 6
					...
					SetButtonState PROPSELECT, gt.titlebar
				Case 7
				...
				Case 10
					...
					ShowGadget PROPSELECT
					SetButtonState PROPSELECT, gt.tool
				Case 11
			...
			Select item
				...
				Case 1
					For count = -1 To gt.parent.itemcount
						AddGadgetItem PROPCOMBO, count
					Next
					SelectGadgetItem PROPCOMBO, gt.tabitem +1
					ShowGadget PROPCOMBO
		...
		Case MENUITEM
			Select editor.activeprop
				Case 0
					...
					ModifyTreeViewNode mt.node, mt.label$
					UpdateCanvas( MENUCANVAS )
				Case 1
...
Function StorePropInput()
	...
	Select gt.version
		Case WINDOW
			Select editor.activeprop
				Case 0
					gt.name$ = TextFieldText( PROPTEXT )
					ModifyTreeViewNode gt.node, gt.name$
				Case 1
				...
				Case 5
					...
					ResizeWindow()
				Case 6
					gt.titlebar = ButtonState(PROPSELECT )
				Case 7
				...
				Case 10
					gt.tool = ButtonState( PROPSELECT )
				Case 11
		...
		Case TABPANEL
			Select editor.activeprop
				Case 0
					...
					ModifyTreeViewNode gt.node, gt.name$
				Case 1
					gt.tabitem = SelectedGadgetItem( PROPCOMBO ) -1
			End Select
...
Function ProjectNew()
	...
	For gt.gtype = EachIn gtype
		If gt <> editor.rootgt Then gt=Null	'Delete gt
	Next
...
'!!	Delete Each mtype
	For Local U = EachIn mtype ; u=Null; Next
	...
	editor.rootmt.node = AddTreeViewNode( "MENUBAR", editor.rootgt.node, MENUROOT )
	...
	If ok
		...
		If filename$ <> ""
			...
			If stream
				...
				' show full tree
				ExpandTreeViewNode gt.node
				ExpandTreeViewNode editor.rootmt.node
...
'			Case "1.0.2"
		...
		mt.node = AddTreeViewNode( mt.label$, parent.node, MENUROOT )
...
Function SaveGadgetList( l:list, stream )
	...
	While l <> Null
		...
		WriteByte stream, gt.textr
		WriteByte stream, gt.textg
		WriteByte stream, gt.textb
		...
		WriteString stream, gt.tips$
		WriteByte stream, gt.itemcount

		' 1.1.0 changes

		WriteByte stream, gt.tabitem
