...
Type TFoldBox
	...
	Field prev:Tfoldbox				' linked list
	Field nxt:Tfoldbox
End Type
...
'!! Added this rather than use the Windows API so that results will be
'!! Cross platform
Function api_GetSysColor:Int( index:Int )
Local color:Int[] = [0,0,0,0,0,$CCCCCC,0,0,$00007F,0,0,0,0,$0000FF,0,0,0,0,$FFFFFF,0,0,0]
Return color[index]
End Function

Function SetUpGUIde()

	GUIde:TGUIde = New GUIde

	GUIde.font = LoadGuiFont("Tahoma",13 )			' alter this if you want to change the font
...
Function CreateFoldBox:TFoldBox( label$, ypos, height, group )
	...
	For b1:Tfoldbox = EachIn foldbox
...
Function FlipFoldBox( b:Tfoldbox )
...
Function ScrollFoldBoxes()
	...
	ElseIf distance > 0	' moving groupboxes up
		...
		While b.nxt <> Null			' find the last foldbox
			b = b.nxt
		Wend
...
Function CreateSpinner:spinner( xpos,ypos, width, height, group, integer, value#, minval#, maxval#, stepval#, prefix$ ="", suffix$ = "" )
	...
	s.spinner = New spinner
		...
		s.textgad = CreateTextField( xpos,ypos,width,height, group )
		...
		SetGadgetText s.textgad, val + s.suffix$
...
Function CheckColorSelectors( canvas )
	...
	For c:CSelector = EachIn CSelector
		If c.canvas = canvas
...
Function LinkToSplitter( gadget%, s:splitter, side% )
	...
	Select side
		...
		Case 2
			s.tcount = s.tcount + 1
			CreateGadgetInt( s.canvas,"top" + s.tcount , gadget )
		Case 3
			s.bcount = s.bcount + 1
