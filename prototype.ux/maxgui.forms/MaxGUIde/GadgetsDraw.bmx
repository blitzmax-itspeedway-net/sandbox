'
' GadgetsDraw.bb
' Include file for GUIDE 1.x
' Gadget draw and helper functions

'#Region Gadget draw functions

Function DrawGadget( gt:gtype )

	' helper function, called by DrawGUIGadget()
	' actually draws the passed gadget

	If gt = Null Then RuntimeError "DrawGadget: gadget does not exist!"
	Local x, y

	gt.drawn = True		' set drawn flag
...
Function DrawGUIGadget( gt:gtype )

	' draws passed gadget and its children
	' but only if not hidden
	' oversized foldboxes are also not drawn
	If gt = Null Then RuntimeError "DrawGUIGadget: gadget does not exist!"
	
	Local draw = True
	If gt.hide = False
		If gt.version = FOLDBOX1
			If gt.ypos + gt.height > gt.parent.ypos + gt.parent.height Then draw = False
		EndIf

		If draw = True
			DrawGadget( gt )

			' draw possible children
			l.list = gt.childlist
			While l <> Null
				DrawGUIGadget( l.gt )
				l = l.nxt
...
'!!		lbl$ = FitStringToWidth( String("*",Len(gt.defaultstring) ), gt.width - 4 )
		lbl$ = FitStringToWidth( "*"[..(Len(gt.defaultstring))], gt.width - 4 )
...
Function GAD_TextArea( gt:gtype )
	...
	Color gt.textr, gt.textg, gt.textb
